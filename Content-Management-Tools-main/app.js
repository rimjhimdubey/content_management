...
const getPass = async () => {
  const namee = await process.env.USER;
const pass = await process.env.PASSWORD;
console.log(namee);
mongoose.connect(`mongodb+srv://${namee}:${pass}@cluster0.5jmf7ef.mongodb.net/blogDB`);
}
getPass();
...
//--MAIN OBJECT FOR THAKUR STORING ALL POSTS IN ARRAY
...
app.post("/compose", function (req, res) {
  ...
  AllPost.findOne({}, function (err, foundObject) {  
    if(!foundObject){
      ...
      const fun = async () => {
        await foundObject.allPostsArray.push(newPost);
